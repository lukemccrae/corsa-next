...
import { LiveStream, User } from "../generated/schema";
...
export default function GroupLiveMap({ runners }: Props) {
  ...
  const createRunnerIcon = (runner: User, index: number) => {
    ...
    <div className="relative w-full h-[500px]">
      ...
      >
        ...
        {runners.map((runner, index) => {
          const color = RUNNER_COLORS[index % RUNNER_COLORS.length];
          const positions = runner.waypoints.map(
              ...
              >
                <Popup>
                  <div className="text-sm">
                    ...
                    {runner.title && (
                      <div className="text-xs">{runner.title}</div>
                    )}
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 w-full text-left px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-xs ${
                  selectedRunner === runner.streamId
              ...
              >
                ...
                />
                <span className="truncate flex-1">{runner.user?.username}</span>
                {runner.live && (
