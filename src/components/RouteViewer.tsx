...
export type RouteViewerProps = {
  route: Route;
  /** Called with the new name after a successful save. Omit to hide edit controls. */
  onRename?: (newName: string) => Promise<void>;
};

export default function RouteViewer({ route, onRename }: RouteViewerProps) {
  ...
  // Reset edit state when the route changes
  useEffect(() => {
    setNameValue(route.name);
    setEditing(false);
  }, [route.storageUrl, route.name]);

  // Load GeoJSON whenever route changes
  useEffect(() => {
    if (!route?.storageUrl) {
      setCoords([]);
      setLoadState("idle");
  ...
  return (
    <div>
      {/* Header: route name with optional inline edit */}
      <div className="flex items-center gap-2 px-3 pt-3 pb-1 sm:px-4 sm:pt-4">
        {editing ? (
          <>
            <InputText
              value={nameValue}
